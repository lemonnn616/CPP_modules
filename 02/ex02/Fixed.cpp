...
Fixed &Fixed::operator++() // vozvrashem silku eto dlya ++a
{
	_rawBits++; // 256
	return *this;
}

Fixed Fixed::operator++(int) // samo znachenie mi uvelichivaem no pechataem kopiu
{
	Fixed tmp(*this);
	++(*this);
	return tmp;
...
Fixed &Fixed::min(Fixed &a, Fixed &b) // < nash operator
{
	return (a < b) ? a : b;
}
